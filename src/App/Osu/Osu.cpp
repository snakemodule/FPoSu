...
Osu::Osu()
{
	...
	*/
	...
	//fposu buffer
	playfieldBuffer = engine->getResourceManager()->createRenderTarget(0, 0, getScreenWidth(), getScreenHeight());
...
void Osu::draw(Graphics *g)
{
	...
	if (isInPlayMode()) // if we are playing a beatmap
	{
		...
		m_shaderGenericTextured->enable();
		{
			...
			playfieldBuffer->bind();
			{
				g->setColor(0xffffffff);
				g->drawVAO(&arrayObject);
			}
			playfieldBuffer->unbind();
...
void Osu::update()
{
	...
	{
		//===================== FPOSU
		if (fposu_invert_horizontal.getBool())
			camera->rotateY(engine->getMouse()->getRawDelta().x);
		else
			camera->rotateY(-engine->getMouse()->getRawDelta().x);
		if (fposu_invert_vertical.getBool())
			camera->rotateX(engine->getMouse()->getRawDelta().y);
		else
			camera->rotateX(-engine->getMouse()->getRawDelta().y);
...
void Osu::rebuildRenderTargets()
{
	...
	playfieldBuffer->rebuild(0, 0, g_vInternalResolution.x, g_vInternalResolution.y);
...
void Osu::makeBackgroundCube() {
	cubeImage = (OpenGLImage*)engine->getResourceManager()->loadImage("backgroundcube.png", "backgroundcube", false);
...
void Osu::makePlayfield() {
	...
	modelMatrix = Matrix4().translate(0, 0, 0).scale(1, (playfieldBuffer->getHeight() / playfieldBuffer->getWidth())*(circumlength), 1);
}
